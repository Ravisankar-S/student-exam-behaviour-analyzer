...
from app.ml.supervised.decision_tree.decision_tree_behavior import DEFAULT_ARTIFACT_DIR, predict_from_dataframe


def main() -> None:
    parser = argparse.ArgumentParser(description="Run predictions using saved Decision Tree behavior artifacts")
    ...
    print(predicted["predicted_label"].value_counts().to_string())

    if "confidence" in predicted.columns:
        print(f"\nAverage confidence: {predicted['confidence'].mean():.4f}")
